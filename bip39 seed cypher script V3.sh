...
# BIP39 wordlist - keeping it in the same file as requested
declare -ra WORDS=(
...
show_usage() {
    cat << 'EOF'
Usage: $0 [-f 'output filename'] [word1 word2 word3 ... | seedfile.txt]
        then script asks 'Enter password' type your seed password and hit enter"

Parameters:
    -f FILENAME     Optional. Output filename (will append .txt if needed)
    WORDS          Space-separated words or input file with words

Examples:
    $0 word1 word2 word3 ...
    $0 -f mycodedseed word1 word2 word3 ...
    $0 mycodedseed.txt
...
validate_output_file() {
    ...
    if [[ -n "$file" && ! -w "$dir" ]]; then
        echo "Error: No write permission in directory $(dirname "$file")" >&2
        exit 1
    fi
}
...
mix_words() {
    ...
    for ((i = size - 1; i > 0; i--)); do
        ...
        temp=""  # Clean sensitive data
    done

    hash=""  # Clean sensitive data
    password=""  # Clean sensitive data
    printf "%s\n" "${shuffled_words[@]}"
}
...
process_words() {
    ...
    local -a mixed_words=($(mix_words "$password"))
    password=""  # Clean sensitive data
    local half_size=$(( ${#mixed_words[@]} / 2 ))
...
main() {
    ...
    PASSWORD=""  # Clean password from memory
