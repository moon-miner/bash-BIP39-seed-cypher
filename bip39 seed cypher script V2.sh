...
show_usage() {
    cat << 'EOF'
Usage: $0 -p 'password' -f 'output filename' [word1 word2 word3 ... | seedfile.txt]

Parameters:
    -p PASSWORD     Required. Password for encryption/decryption
    -f FILENAME     Optional. Output filename (will append .txt if needed)
    WORDS          Space-separated words or input file with words

Examples:
    $0 -p password1 -f mycodedseed word1 word2 word3
    $0 -p password1 mycodedseed.txt
